...
@onready var talent_points_label: Label = $SafeArea/Top/Content/Meta/TalentPoints
...
func _ready() -> void:
	...
	_on_talent_points_changed(TalentManager.available_points(), TalentManager.earned_points, TalentManager.spent_points)
...
func _on_talent_points_changed(available: int, _earned: int, _spent: int) -> void:
	talent_points_label.text = "TP %d" % available
	talent_points_label.visible = available > 0
